...
try:
    while (time.time() - t_init) < 60:  # Run for 10 seconds
        # Send data
        # ser.write(b"Hello from Raspberry Pi!\n")
        # ser.write(str.encode(str(a)) + str.encode("\n"))
        # data_to_send = f"{a},{b}\n"
        if time.time() - t_init < 15.0:
            data_to_send = 'off_start' + '\n'
            ser.write(data_to_send.encode('utf-8'))
        elif time.time() - t_init < 30.0:
            data_to_send = 'off_end' + '\n'
            ser.write(data_to_send.encode('utf-8'))
        elif time.time() - t_init < 45.0:
            data_to_send = 'on_start' + '\n'
            ser.write(data_to_send.encode('utf-8'))
        else:   
            data_to_send = 'on_end' + '\n'
            ser.write(data_to_send.encode('utf-8'))
